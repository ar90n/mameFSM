...
template<typename T>
concept TransitionType = is_transition_type<std::decay_t<T>>::value || 
                        is_variant_of_transitions<std::decay_t<T>>::value;
...
struct FSM {
    ...
    class Runner {
        ...
        void switch_to(Context& ctx, Args&&... a) {
            std::visit([&]<typename Cur>(Cur& s){
                if constexpr (!std::same_as<Cur, std::monostate>) s.on_exit(ctx);
            }, current_);
        ...
        void apply_transition(Context& ctx, TrType&& tr) {
            ...
            static_assert(contains_v<Next, States...>,
                          "Transition target not in FSM states");

            if constexpr (!std::same_as<Next, Cur>) {
                std::apply([&](auto&&... a){
                    switch_to<Next>(ctx, std::forward<decltype(a)>(a)...);
                }, tr.args);
            }
        }

    public:
        explicit Runner(Context& ctx) { switch_to<InitialState>(ctx); }

        void update(Context& ctx) {
            std::visit([&]<typename Cur>(Cur& cur_state) {
                if constexpr (std::same_as<Cur, std::monostate>) {
                    ...
                    if constexpr (is_transition_type<Tr>::value) {
                        ...
                        std::visit([&]<typename ActualTr>(ActualTr&& actual_tr) {
        ...
        void force_transition(Context& ctx) { switch_to<Target>(ctx); }

        template<typename T>
        [[nodiscard]] bool is_in_state() const { return std::holds_alternative<T>(current_); }
    };
};
